...
import { ChevronDown, ChevronsUpDown, Ellipsis, Send } from 'lucide-react';
...
interface ProposalRow {
  ...
  sender: string;
  status: 'Sent' | 'Accepted' | 'Declined' | 'Archived';
}

function makeProposals(count: number): ProposalRow[] {
  const statuses: ProposalRow['status'][] = [
    'Sent',
    'Sent',
    'Accepted',
    'Declined',
...
const allProposals = makeProposals(337);
const ROW_OPTIONS = [10, 20, 50, 100];

function getProposalBadgeClass(status: ProposalRow['status']): string {
  switch (status) {
    case 'Sent':
...
export default function ProposalTab() {
  ...
  const pageCount = Math.ceil(allProposals.length / rowsPerPage);
  const paginated = allProposals.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
  ...
  return (
    <div className="">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl">Proposal & Agreement</h2>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem>New Agreement</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
                ...
                <TableRow key={row.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="py-4">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-gray-700 text-[10px]">
                        NU
                      </span>
                  ...
                  <TableCell className="py-4">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button
                          className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-sm font-medium rounded-sm transition-colors ${getProposalBadgeClass(
                            row.status
                          )}`}
                        >
                          {row.status}
        ...
        <div className="flex items-center justify-between mt-6 text-sm">
          <div className="flex items-center gap-6">
            ...
            <div className="text-sm">
              ...
              {Math.min(page * rowsPerPage, allProposals.length)} of{' '}
              {allProposals.length}
            </div>
          </div>
